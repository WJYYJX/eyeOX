...
def read_image(image, mask):
    ...
    :param mask: path to mask (either nrrd or png format)
    ...
    if (np.unique(mask) == [0, 1, 2, 3]).all():
        pass
    else:
        return

    if (np.array(mask.shape) == np.array(image.shape[:2])).all():
        pass
    else:
        mask = mask.T
        if (np.array(mask.shape) == np.array(image.shape[:2])).all():
...
def get_oxy(red, green, labels_im, labels_im1, flag=True):
    map_ske = skeletonize(np.ascontiguousarray(labels_im.astype(bool)))  #
...
img1 = r"C:\Users\111\Desktop\0\OD_1.jpg"
mask = r"C:\Users\111\Desktop\0\OD_1_Origin.png"
